...
graph TD
  ...
  %% Hue Models <--> RGB (using sRGB as hub)
  HSI <--> sRGB
  HSL <--> sRGB
  HSV <--> sRGB
  HWB <--> sRGB
...
# Folder structure (TMP)
src/
└── models/
    ├── rgb/
    │   └── rgb255/
    │       ├── index.ts        // Exports definition, converter, parser
    │       ├── rgb255.model.ts       // Model definition (class)
    │       ├── rgb255.converter.ts
    │       └── rgb255.parser.ts
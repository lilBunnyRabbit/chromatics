...
// Bind theme roles to named colors — one \`roles { }\` block (role = color).
// The editor stays the source of truth; \`theme({ … })\` still works too.
roles {
  bg          = background
  fg          = foreground
  surface     = surface
...
// \`token(name, { … })\` defines any other group. (Flat \`tokens.text(…)\` works too.)
tokens {
  text   = text(16, 1.25)
  space  = space(4)
  radius = radius(10)
...
// Color fields take a role/named color (with an optional /opacity); size fields
// take a bare token step ("lg" → radius.lg, "4" → space.4).
component {
  button = button({
    variants: [

...
export type XYZIlluminant = [x2: number, y2: number, z2: number, x10: number, y10: number, z10: number];
...
} satisfies Record<string, XYZIlluminant>;
...
class XYZBase extends Float32Array {
   ...
   * @param z - [0, 1]
   */
  constructor(x: number, y: number, z: number, readonly illuminant: keyof typeof illuminants = "D65") {
    super(3);
  ...
  public toString() {
    ...
    let type = "xyz";
    if (this.illuminant === "D50") type += "-d50";
    else if (this.illuminant === "D65") type += "-d65";
...
class XYZConversions extends XYZBase {
  ...
  public toLAB(): Lab {
    ...
    return new Lab(l, a, b, this.illuminant);
  }
...
export class XYZ extends XYZConversions {
  static sum(...xyzs: XYZ[]): XYZ {
    ...
    return new XYZ(x, y, z, xyzs[0].illuminant);
  }
}

...
import registerParsers from "../models/model.parser";
...
export type ParserFunction<To, Args extends unknown[] = []> = (input: string, ...args: Args) => To | undefined | null;
...
export class ParserRegistry {
  ...
  public register<To, Args extends unknown[] = []>(target: Constructor<To>, spec: ParserSpec<To, Args>) {
  ...
  public getParsersFor<To>(target: Constructor<To>): ParserSpec<To, any>[] {
    return this.registry.get(target) || [];
  }

  // Try to parse the input string for a specific target model.
  // Returns the first successful result or undefined.
  public parseFor<To>(target: Constructor<To>, input: string): To | undefined {
    ...
    for (const [target, specs] of this.registry.entries()) {
      for (const spec of specs) {
        ...
        try {
          const result = spec.parser(input);
          if (result !== undefined) {
            results.push({ target: (target as Constructor<any>).name, result });
          }
        } catch (error) {

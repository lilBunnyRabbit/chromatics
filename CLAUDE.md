...
bun test               # unit tests (bun:test) — currently 335 across 27 files
...
- `scheme/` — `adapter` (`EvalResult`→`Scheme`), `roles` (role mapping + audit), `tokens`, `components`, `theme-config` (mode-aware: `themeRolesFromScheme(scheme, mode)`). `derive.ts` is the **shared spine** (CD-12): `deriveScheme(source, {roles, opacities}) → RenderedScheme` bundles the whole `evaluate → scheme → roles/audit/tokens/components` cascade (plus the `dark*` siblings) as one pure function; `app.svelte.ts` calls it once and re-exposes each field as a thin `$derived`. `flow.ts` is the **Design System** guided-flow model (pure step completeness + scaffold strings). `modes.ts` does light/dark as pure role re-binding (`darkRolesFor`/`modeWarnings`/`modeFragilePairs`).
